...
void computingWaitingTime(int *arr, int n, int m) {
    ...
    int cur_time = *(input_arr + 1);
    int *temp_arr = NULL;
    int wt0 = *(input_arr + 1);

    for (int i = 0; i < n; i++) {
        k = 0;
        for (int j = i; j < n && *(input_arr + j * m + 1) <= cur_time; ++j) {
            k++;
        }
        temp_arr = (int *) malloc(k * 3 * sizeof(int));
        int q = 0;
        for (int l = 0; l < n; ++l) {
            if (*(input_arr + l * m + 0) != -1 && *(input_arr + l * m + 1) <= cur_time) {
                for (int j = 0; j < 3; ++j) {
                    *(temp_arr + q * 3 + j) = *(input_arr + l * m + j);
                }
                q++;
            }
        }

        sorting_by_BT(temp_arr, k, 3);

        cur_time  = cur_time+ *(temp_arr + 2);

        for (int j = 0; j < m; j++) {
            *(arr + i * m + j) = *(temp_arr + j);
        }
        *(input_arr + (*temp_arr - 1) * m + 0) = -1;
